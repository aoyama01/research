...
void cmat(double k);
double c1,c2;
double *loc_sum0_1,*loc_sum1_1,*loc_sum2_1;
double *loc_sum0_2,*loc_sum1_2,*loc_sum2_2;
...
main(int argc, char **argv)
{
	...
	for(i=1;i<=nr;i++){
		fprintf(stdout,"%ld,%lf,%lf,%lf,%lf,%lf,%lf\n",rs[i],F2_12[i],log((double)rs[i]/1.93)/log(10),log(fabs(F2_12[i]))/log(10)/2,F2_12[i]/(F_11[i]*F_22[i]),log(F_11[i])/log(10),log(F_22[i])/log(10));
...
double est_f2(long n, long k2){
	double f2,a0_1,a0_2;
	double y00_1,y0i_1,y0ii_1,y10_1,y1i_1,y1ii_1;
	double y00_2,y0i_2,y0ii_2,y10_2,y1i_2,y1ii_2;
	double temp1,temp2;
	double nn1,nn2;
	...
	cmat((double)k);
	a0_1 = c1*y10_1+c2*y1ii_1;
	temp1 = (y01[ic] - a0_1);
	a0_2 = c1*y10_2+c2*y1ii_2;
	temp2 = (y02[ic] - a0_2);
	f2 = temp1*temp2;
	f2_11 = temp1*temp1;
	f2_22 = temp2*temp2;
	...
	for(i=1;i<=itemp;i++){
		if(i % i_refresh == 0){
			...
			y1ii_1 = y0ii_1 - 2*y0i_1 + y00_1 - temp1 *(double)(k+1) + temp2*(double)k;

			y10_2 = y00_2 + y02[i+scale]-y02[i];
			temp1 = y02[i]*(double)(k+1);
			temp2 = y02[i+scale]*(double)k;
			y1i_2 = y0i_2 - y00_2 + temp1 + temp2;
			y1ii_2 = y0ii_2 - 2*y0i_2 + y00_2 - temp1 *(double)(k+1) + temp2*(double)k;
